...
TRexMaterials::TRexMaterials() {
	...
	fIso_H2 = new G4Isotope("H2",1,2,2.014*CLHEP::g/CLHEP::mole);
	fElD = new G4Element("Deuterium Atom","D",1);
	fElD->AddIsotope(fIso_H2, 100.*CLHEP::perCent);
	...
	G4Material* c2h6 = new G4Material("c2h6", 1.290 * CLHEP::mg/CLHEP::cm3, 2);
	...
	G4Material* plasticFoam = new G4Material("PlasticFoam", 0.15 * CLHEP::g/CLHEP::cm3, 3);
	plasticFoam->AddElement(fElC, 2);
	plasticFoam->AddElement(fElH, 2);
	plasticFoam->AddElement(fElO, 1);

	G4Material* carbonFibre = new G4Material("CarbonFibre", 1.8 * CLHEP::g/CLHEP::cm3, 2);
	carbonFibre->AddElement(fElC, 10);
	carbonFibre->AddElement(fElN, 1);

	G4Material* carbonLH2 = new G4Material("carbonLH2", 2.073 * CLHEP::g/CLHEP::cm3, 2);
	carbonLH2->AddElement(fElC, 10);
	carbonLH2->AddElement(fElN, 1);

	G4Material* epoxy = new G4Material("epoxy", 1.2 * CLHEP::g/CLHEP::cm3, 3);
	...
	G4Material* stainlessSteel = new G4Material("Stainless Steel",
			7.9 * CLHEP::g/CLHEP::cm3, 3, kStateSolid, 50.0 * CLHEP::kelvin, 1.0 * CLHEP::atmosphere);
	...
	fManager->BuildMaterialWithNewDensity("airexC70", "G4_POLYVINYL_CHLORIDE", 40 * CLHEP::kg/CLHEP::m3);
	...
	G4Material* cf4 = new G4Material("CF4", 3.72 * CLHEP::mg/CLHEP::cm3, 2);
	cf4->AddElement(fElC, 1);
	cf4->AddElement(fElF, 4);

	G4Material* g11 = new G4Material("G11", 1.9 * CLHEP::g/CLHEP::cm3, 3);
	g11->AddElement(fElC, 5);
	g11->AddElement(fElH, 8);
	g11->AddElement(fElO, 2);

	G4Material* dc4gas = new G4Material("dc4gas", 1.65 * CLHEP::mg/CLHEP::cm3, 3); // density = 1.37 * density_air
	dc4gas->AddMaterial(argon, 0.45);
	dc4gas->AddMaterial(ethan, 0.45);
	dc4gas->AddMaterial(cf4, 0.1);

	G4Material* strawGas = new G4Material("strawGas", 2.08 * CLHEP::mg/CLHEP::cm3, 3);
	strawGas->AddMaterial(argon, 0.74);
	strawGas->AddMaterial(co2, 0.06);
	strawGas->AddMaterial(cf4, 0.20);

	G4Material* w45Gas = new G4Material("w45Gas", 1.88 * CLHEP::mg/CLHEP::cm3, 3);
	w45Gas->AddMaterial(argon, 0.85);
	w45Gas->AddMaterial(co2, 0.05);
	w45Gas->AddMaterial(cf4, 0.10);

	G4Material* mw1Gas = new G4Material("mw1Gas", 1.716 * CLHEP::mg/CLHEP::cm3, 2);
	mw1Gas->AddMaterial(argon, 0.7);
	mw1Gas->AddMaterial(co2, 0.3);

	G4Material* mw2Gas = new G4Material("mw2Gas", 1.41 * CLHEP::mg/CLHEP::cm3, 2); // density = 1.17 * density_air
	mw2Gas->AddMaterial(argon, 0.75);
	mw2Gas->AddMaterial(ethan, 0.25);

	G4Material* ecalShashlik = new G4Material("ecalShashlik", 4.639 * CLHEP::g/CLHEP::cm3,
			2);
	ecalShashlik->AddMaterial(lead, 0.348);
	ecalShashlik->AddMaterial(polystyrene, 0.652);

	G4Material* ecalMainz = new G4Material("ecalMainz", 5.176 * CLHEP::g/CLHEP::cm3, 2);
	ecalMainz->AddMaterial(lead, 0.4);
	ecalMainz->AddMaterial(polystyrene, 0.6);

	G4Material* ecalOlga = new G4Material("ecalOlga", 4.49 * CLHEP::g/CLHEP::cm3, 2);
	ecalOlga->AddMaterial(lead, 0.33);
	ecalOlga->AddMaterial(polystyrene, 0.67);

	G4Material* hcal1Material = new G4Material("hcal1Material", 6.51 * CLHEP::g/CLHEP::cm3,
			2);
	hcal1Material->AddMaterial(iron, 0.8);
	hcal1Material->AddMaterial(polystyrene, 0.2);

	G4Material* hcal2Material = new G4Material("hcal2Material", 10.25 * CLHEP::g/CLHEP::cm3,
			2);
	hcal2Material->AddMaterial(lead, 0.893);
	hcal2Material->AddMaterial(polystyrene, 0.107);

	G4Material* kapton = new G4Material("Kapton", 1.42 * CLHEP::g/CLHEP::cm3, 4);
	...
	G4Material* rohacell = new G4Material("rohacell", 51.3 * CLHEP::kg/CLHEP::m3, 4); // C9 H13 N O2  0.0513 g/cc
	rohacell->AddElement(fElC, 9);
	rohacell->AddElement(fElH, 13);
	rohacell->AddElement(fElN, 1);
	rohacell->AddElement(fElO, 2);

	G4Material* innox = new G4Material("innox", 7.90 * CLHEP::g/CLHEP::cm3, 3);
	...
	G4Material* gemMaterial = new G4Material("gem-material", 4.49 * CLHEP::g/CLHEP::cm3, 2);
	gemMaterial->AddMaterial(kapton, 0.91);
	gemMaterial->AddMaterial(copper, 0.09);

	G4Material* mwpcGas = new G4Material("mwpcGas", 2.08 * CLHEP::mg/CLHEP::cm3, 3);
	mwpcGas->AddMaterial(argon, 0.74);
	mwpcGas->AddMaterial(co2, 0.06);
	mwpcGas->AddMaterial(cf4, 0.20);

	G4Material* mwpcMaterial = new G4Material("mwpc-material", 4.49 * CLHEP::g/CLHEP::cm3, 2);
	mwpcMaterial->AddMaterial(mwpcGas, 0.893);
	mwpcMaterial->AddMaterial(polystyrene, 0.107);

	G4Material* micromegasGas = new G4Material("micromegasGas", 0.95 * CLHEP::mg/CLHEP::cm3, 3);
	...
	G4Material* richwallGas = new G4Material("w45Gas", 2.08 * CLHEP::mg/CLHEP::cm3, 3);
	...
	G4Material* fiberGlass = new G4Material("fiberGlass", 2.5 * CLHEP::g/CLHEP::cm3, 5);
	...
	G4Material* pcb = new G4Material("pcb", 1.8 * CLHEP::g/CLHEP::cm3, 3);
	...
	fManager->ConstructNewGasMaterial("chamberGas", TRexSettings::Get()->GetVacuumChamberGas(), CLHEP::STP_Temperature, TRexSettings::Get()->GetVacuumChamberGasPressure());
}

void TRexMaterials::DefineIndex() {
	fBc408Index = 1.58;
	fBc408AbsorptionLength = 3.80 * CLHEP::m; // bulk light attenuation length
	fBc408Yield = 0.64 * 2 / (100 * CLHEP::eV); // 64% anthracene
	fPlexiglassIndex = 1.49;
	fPlexiglassAbsLength = 10.0 * CLHEP::m;
	fGlassIndex = 1.474;
	...
	fPhotonEnergyShort[0] = 2.0 * CLHEP::eV; // 619.92 nm
	fPhotonEnergyShort[1] = 7.14 * CLHEP::eV; // 173.65 nm
	fPhotonEnergyLong[0] = 2.08 * CLHEP::eV; // 596.08 nm
	fPhotonEnergyLong[1] = 2.38 * CLHEP::eV; // 520.94 nm
	fPhotonEnergyLong[2] = 2.58 * CLHEP::eV; // 480.56 nm
	fPhotonEnergyLong[3] = 2.70 * CLHEP::eV; // 459.20 nm
	fPhotonEnergyLong[4] = 2.76 * CLHEP::eV; // 449.22 nm
	fPhotonEnergyLong[5] = 2.82 * CLHEP::eV; // 439.66 nm
	fPhotonEnergyLong[6] = 2.92 * CLHEP::eV; // 424.60 nm
	fPhotonEnergyLong[7] = 2.95 * CLHEP::eV; // 420.29 nm
	fPhotonEnergyLong[8] = 3.02 * CLHEP::eV; // 410.54 nm
	fPhotonEnergyLong[9] = 3.10 * CLHEP::eV; // 399.95 nm
	fPhotonEnergyLong[10] = 3.26 * CLHEP::eV; // 380.32 nm
	fPhotonEnergyLong[11] = 3.44 * CLHEP::eV; // 360.42 nm

	fAirRefractiveIndex[0] = 1.0;
	fAirRefractiveIndex[1] = 1.0;
	fAirAbsorpiton[0] = 10.0 * CLHEP::m;
	fAirAbsorpiton[1] = 10.0 * CLHEP::m;
...
void TRexMaterials::BuildAir() {
	...
	G4Material* vacuum = new G4Material("Vacuum", 1.0e-5 * 1.290 * CLHEP::mg/CLHEP::cm3, 2);
...
G4Material* TRexMaterials::BuildBc408() {
	...
	G4Material* bc408 = new G4Material("Scintillator", 1.032 * CLHEP::g/CLHEP::cm3, 2);
	...
	bc408MaterialProperties->AddConstProperty("FASTTIMECONSTANT", 0.9 * CLHEP::ns); // rise time
	bc408MaterialProperties->AddConstProperty("SLOWTIMECONSTANT", 2.1 * CLHEP::ns); // decay time
...
G4Material* TRexMaterials::BuildAluminium() {
	...
	G4double aluminiumAbsorpiton[nEntryShort] = { 1.0e-20 * CLHEP::m, 1.0e-20 * CLHEP::m };
...
G4Material* TRexMaterials::BuildBialkali() {
	G4Material* bialkali = new G4Material("bialkali", 3 * CLHEP::g/CLHEP::cm3, 3, kStateSolid);
	...
	G4double bialkaliAbsorption[nEntryShort] = { 1.0e-6 * CLHEP::mm, 1.0e-6 * CLHEP::mm };
...
G4Material* TRexMaterials::BuildC4F10() {
	G4Material* c4f10 = new G4Material("C4F10", 11.21 * CLHEP::kg/CLHEP::m3, 2, kStateGas, 298 * CLHEP::kelvin, 1.0 * CLHEP::atmosphere);
	...
	c4f10MaterialProperties->AddConstProperty("SCINTILLATIONYIELD", 100.0/CLHEP::MeV);
...
G4Material* TRexMaterials::BuildDeuteratedPE() {
	G4Material* deuteratedPE = new G4Material("DeuteratedPE", 0.92 * CLHEP::g/CLHEP::cm3, 2);
...
G4Material* TRexMaterials::BuildSolidDeuterium() {
	G4Material* solidDeuterium = new G4Material("SolidDeuterium", 196.7 * CLHEP::kg/CLHEP::m3, 1, kStateSolid);
...
G4Material* TRexMaterials::GetMaterial(std::string name) {//, double pressure) {
	...
	if(name == "vacuum" || name == "dummy") {
		...
		G4double molarMass = TRexMaterials::Get()->GetNistManager()->GetAtomicMass(bufferMaterial->GetZ(),massNumber)*(CLHEP::g/CLHEP::mole)/CLHEP::amu_c2;
		G4cout << "matName " << matName << " massNrString " << massNumber << " elementName " << elementName << " Z " << bufferMaterial->GetZ() <<" molar mass " << std::setprecision(5)<<
			molarMass/(CLHEP::g/CLHEP::mole) << " density " << density/(CLHEP::g/CLHEP::cm3)<< G4endl;
